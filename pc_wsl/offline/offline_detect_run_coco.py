...
def ffprobe_info(video_path: Path) -> VideoInfo:
    ...
    s = info["streams"][0]
    return VideoInfo(width=int(s["width"]), height=int(s["height"]))

def ffprobe_frame_times(video_path: Path) -> List[float]:
    """
    Per-frame best_effort_timestamp_time (PTS in seconds).
    This aligns our event times with ffmpeg -ss seeking (used by Evidence Builder).
    """
    cmd = [
        "ffprobe", "-v", "error",
        "-select_streams", "v:0",
...
def ffmpeg_frames_rgb(video_path: Path, width: int, height: int):
    ...
    NOTE:
    - We silence stderr to avoid noisy "Broken pipe" logs when the reader stops early.
    - We terminate/kill ffmpeg cleanly in finally.
    """
    cmd = [
    ...
    proc = subprocess.Popen(
        ...
        stderr=subprocess.DEVNULL,  # important: suppress "Broken pipe" spam
        bufsize=10**8
    )
    ...
    try:
        while True:
            raw = proc.stdout.read(frame_bytes) if proc.stdout is not None else b""
        ...
        # Close stdout so ffmpeg can notice the pipe is closed.
        try:
            if proc.stdout:
                proc.stdout.close()
        except Exception:
        ...
        # Terminate ffmpeg (if still alive) and wait briefly.
        try:
...
def load_roi_polygon(path: Optional[str], W: int, H: int) -> List[Tuple[float, float]]:
    if not path:
...
def main():
    ap = argparse.ArgumentParser(
        description="Offline MP4 -> events.json (EB compatible, PTS-aligned, snapshot-refined bbox)"
    )
    ...
    ap.add_argument("--conf", type=float, default=0.55, help="Detection confidence used to OPEN/MAINTAIN events")
    ap.add_argument("--rep_conf", type=float, default=0.25, help="Confidence used to pick representative bbox on EB snapshot frame (mid_s)")
    ap.add_argument("--min_area", type=float, default=12000.0)
    ap.add_argument("--confirm_n", type=int, default=4)
    ap.add_argument("--end_miss_m", type=int, default=8)
    ...
    # Used when ffprobe returns fewer timestamps than frames decoded.
    # (This avoids early breaks that lead to ffmpeg "broken pipe" spam.)
    dt_fallback = (times[-1] - times[-2]) if len(times) >= 2 else (1.0 / 30.0)

    roi_poly = load_roi_polygon(args.roi, W, H)
    roi_id = Path(args.roi).stem if args.roi else "fullframe"
    ...
    def close_event(end_t: float):
        ...
        # Extract the exact snapshot frame EB will use (midpoint) and refine bbox on that frame.
        tmp = Path(f"/tmp/offline_rep_{video_path.stem}_{ev_idx:04d}.jpg")
        best = None
        try:
            ...
            if tmp.is_file():
                ...
                print(f"[warn] snapshot not created: {tmp}")
        except Exception as e:
            print(f"[warn] snapshot refine failed: {e}")
        finally:
            try:
                tmp.unlink(missing_ok=True)
            except Exception:
                pass
        # If no bbox exists on the snapshot frame, drop the event as a false positive.
        if best is None:
            in_event = False
            miss = 0
            seen = 0
        ...
        events.append({
            ...
            # Make EB show FOD instead of "unknown"
            "class_name": str(model.names.get(rep_cls, rep_cls)),
    ...
    # Stream frames for event timing only (opening/closing events)
    for frame_idx, frame_rgb in ffmpeg_frames_rgb(video_path, W, H):
        if args.max_frames and frame_idx >= args.max_frames:
            break

        # Time mapping: do NOT break if times[] is shorter than decoded frames.
        if frame_idx < len(times):
            t = times[frame_idx]
        else:
            t = times[-1] + (frame_idx - (len(times) - 1)) * dt_fallback
